...
coviddata <- fromJSON(response, flatten = TRUE)$data #%>% data.frame()
...
pgcounty_visits_cli <- pgcounty_cli %>% 
  inner_join(pgcounty_visits, by='date') %>%
  pivot_longer(-date)

ggplot(pgcounty_visits_cli, aes(x = date, y = value)) + 
