...
```{r}
cc_2018 %>% ggplot(aes(x = arrest, fill = primary_type)) +
  geom_bar()
```
...
```{r}
ggplot(cc_sub) +
  geom_point(aes(x = assault, y = theft, color = arrest), size = 1)
```
...
map <- get_stadiamap(bbox = bbox, zoom = 12, maptype = "stamen_toner")
ggmap(map)
```
...
```{r}
ggmap(map) + 
  stat_density2d(data = cc_2018, aes(x = longitude, y = latitude, fill = after_stat(level), alpha = after_stat(level)), size = 1, bins = 40, geom = 'polygon') +
