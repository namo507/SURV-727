...
Unfortunately, there seems to be a problem with the API response when looking up data for sub regions: https://github.com/PMassicotte/gtrendsR/issues/364 

```{r}
crime_shooting_trends <- gtrends(c("crime", "shooting"), 
                                 geo = "US", 
                                 onlyInterest= TRUE
                                 )
# res <- gtrends(c("crime", "shooting"), 
# geo = "US-602", onlyInterest= TRUE)

plot(crime_shooting_trends)
```

The default time horizon is today minus five years. This can be adjusted by using the `time` argument.

```{r}
# res <- gtrends(c("crime", "shooting"), geo = "US-602",
# time = "2018-01-01 2018-06-30", low_search_volume = T)
res <- gtrends(c("crime", "shooting"), geo = "US", 
               time = "2022-01-01 2022-06-30", low_search_volume = T)

plot(res)
```

Information on what is plotted on the y axis can be found in the following articles and in

https://www.aeaweb.org/conference/2016/retrieve.php?pdfid=772
...
The popularity measure `hits` in the original data.frame is a single vector, with another vector `keyword` representing the corresponding search term. We can `spread()` the `hits` information into two columns, based on `keyword`.
...
To reverse this transformation, `gather()` can be used. Note that we use the pipe to pipe-in the tibble as the first argument of `gather()`, i.e. here we begin directly with the `key` argument.

```{r}
res_time_w %>%
  gather(key = "keyword", value = "hits", 6:7)
```
...
```{r}
res_time_w %>%
  subset(date >= "2022-01-15") %$%
  mean(crime)
```
```{r}
res_time_subset <- res_time_w %>% subset(date >= "2022-01-15")
mean(res_time_subset$crime)
```
...
By default, the trend query looks at Google web searches. We can also run the query for other Google products, such as Google news (or images, shopping ("froogle"), youtube). 

```{r}
# res_new <- gtrends(c("crime", "shooting"), 
# geo = "US-602", time = "2018-01-01 2018-06-30",
# low_search_volume = T, gprop = "news")
res_new <- gtrends(c("crime", "shooting"), 
                   geo = "US", time = "2022-01-01 2022-06-30", 
                   low_search_volume = T, gprop = "news")
...
res_join <- # (6)
  ...
  select(date, keyword, hits_news) %>% # (3)
...
Note that the original results object `res_new` contains some additional information, such as the search interest by city.

```{r}
res_new$interest_by_city
```
