...
```
In Q1's result, we see an interesting pattern from April 2020 to July 2020. In April, there was a huge spike in the search hit of Loan but later on in July, there is a shift in the search hit as we see a spike in Crime. So the mean for crime and loans is 51.03774 and 62.24528 respectively. So we can infer that the results indicated significant variability in search intensity for both terms, with noticeable spikes during certain periods in 2020, reflecting societal events like the pandemic and its economic consequences.

Now by aggregating search hits by location, we identified the cities in Illinois with the highest search frequency for "loans." where Long Lake is in the top. This analysis actually provides insights into the geographic distribution of search interest, revealing patterns in financial behavior across different regions.

Now the correlation between search intensities for "crime" and "loans" after computation, reveals whether increases in one term's search frequency corresponded with changes in the other. A moderately high negative correlation of (-0.81) is observed, suggesting that as search interest in "loans" increased, interest in "crime" slightly decreased.
...
Now there's a significant mismatch between Google Trends data and ACS data as 1406 cities aren't appearing in both the datasets, which could be due to differences in how locations are defined or reported in each dataset.

Search popularity by income group is given as
High income:
Mean crime searches: 31.2
Mean loans searches: 30.7
Low income:
Mean crime searches: 25.4
Mean loans searches: 45.1
So we can infer that higher income areas show slightly higher search interest in both crime and loans. This could indicate greater awareness or concern about these issues in more affluent areas, or simply reflect higher internet usage.

The scatter plot shows a slight negative relationship between household income and search popularity for "loan" and a slight positive relationship with keyterm "crime".
This suggests that as household income increases, interest in loans slightly decreases, possibly due to greater financial literacy or interest in investment opportunities.
...
```
So seeing the results, we can say that COVID-related searches were much more frequent and consistent throughout the year, while vaccine searches were less frequent but had higher variability, possibly due to spikes in interest as vaccine development progressed.

Another interesting thing that can be inferred is that larger cities and university towns show the highest search frequencies for both terms, likely due to population size and concentration of health-conscious or academically-oriented residents.
