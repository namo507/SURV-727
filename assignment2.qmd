...
```{r fig.height=8, fig.width=12, message=FALSE, warning=FALSE}
# Question 1
head(res2$interest_over_time, n = 5)
sum_res2_all <- res2$interest_over_time %>% group_by(keyword) %>%
  summarize(mean = mean(hits),
            median = median(hits),
            variance = var(hits))
sum_res_all
...
# Question 2
head(res2$interest_by_city, n = 5)
freq_res2_cities <- res2$interest_by_city %>%select(c(location, keyword, hits))%>%group_by(keyword, location) %>% summarize(total_hits = sum(hits)) %>% ungroup() %>% filter(total_hits > 0) 

head(arrange(freq_res2_cities, desc(total_hits)), n = 10)

freq_res2_city_spread <- spread(freq_res2_cities, key = keyword, value = total_hits)%>% 
  mutate(across(where(is.numeric), ~ replace_na(.,0)))
head(freq_res2_city_spread, n = 5)
freq_res2_city_spread %>% filter(covid > 0 & vaccine > 0)

ggplot(freq_res2_cities, aes(x = location, y = total_hits, fill = keyword))+
  geom_bar(stat = 'identity', position = 'dodge', color = 'black', width = 0.65)+
  labs(title = "Total Hits by Location Colored by Keyword",
       x = "Location", 
       y = "Total Hits",
...
# Question 3
corel <- cor(freq_res2_city_spread$covid, freq_res2_city_spread$vaccine)
cat("The correlation between crime and loan:\t", corel)
cor.test(freq_res2_city_spread$covid, freq_res_city_spread$vaccine)
```
